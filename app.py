...
# Ajoutez ensuite les pages pour freelance et tse
    ...
    try:
        ...
        # Afficher le PDF avec Streamlit
        st.markdown(f"""
        <div style="background: #f9f9f9; padding: 20px; border-radius: 10px; margin-bottom: 30px;">
            <div style="color: #666; margin-bottom: 10px;">Guide mise en page 2021</div>
        </div>
        """, unsafe_allow_html=True)
        
        # Afficher le PDF (remarque: Streamlit supporte l'affichage de PDF)
        st.download_button(
            label="📥 Télécharger le PDF",
            data=pdf_bytes,
            file_name="Guide mise en page2021.pdf",
            mime="application/pdf"
        )
        
        # Optionnel: Afficher un aperçu du PDF si vous avez une librairie pour ça
        # st.write("Aperçu du PDF:")
        ...
        st.markdown("""
        <div style="text-align: center; padding: 40px; background: #f9f9f9; border-radius: 10px; margin-bottom: 30px;">
            <div style="color: #888;">PDF non disponible</div>
        </div>
        """, unsafe_allow_html=True)

    # Section 2: Afficher les 12 images
    st.markdown("""
    ...
    # Affichage des 12 images en 4 lignes de 3 colonnes
    for row in range(0, 12, 3):
        cols = st.columns(3)
        for col_idx in range(3):
            img_idx = row + col_idx
            if img_idx < len(sportswear_images):
                with cols[col_idx]:
                    try:
                        img_url = get_image_url(sportswear_images[img_idx])
                        
                        # Créer un conteneur cliquable pour agrandir l'image
                        st.markdown(f"""
                        <div style="text-align: center; margin-bottom: 20px; cursor: pointer;" 
                             onclick="document.getElementById('modal-{img_idx}').style.display='block'">
                            <img src="{img_url}" 
                                 style="width: 100%; max-width: 300px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); transition: transform 0.3s ease;"
                                 onmouseover="this.style.transform='scale(1.02)'"
                                 onmouseout="this.style.transform='scale(1)'"
                                 alt="SportsWear Design {img_idx + 1}">
                            <div style="color: #666; font-size: 14px; margin-top: 8px;">Design {img_idx + 1}</div>
                        </div>
                        
                        <!-- Modal pour agrandir l'image -->
                        <div id="modal-{img_idx}" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.9);">
                            <div style="position: relative; width: 100%; height: 100%;">
                                <span onclick="document.getElementById('modal-{img_idx}').style.display='none'" 
                                      style="position: absolute; top: 20px; right: 35px; color: white; font-size: 40px; font-weight: bold; cursor: pointer; z-index: 1001;">
                                    &times;
                                </span>
                        ...
                        <div style="text-align: center; margin-bottom: 20px;">
                            <div style="padding: 40px; background: #f9f9f9; border-radius: 10px;">
                                <div style="color: #888;">SportsWear Design {img_idx + 1}</div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)

    # Script JavaScript pour fermer les modales en cliquant à l'extérieur
    st.markdown("""
    <script>
