...
# Ajoutez ensuite les pages pour freelance et tse
    ...
    try:
        # Testons plusieurs chemins possibles
        pdf_paths_to_test = [
            "SportsWear Design/Guide mise en page2021.pdf",
            "mes_documents/SportsWear Design/Guide mise en page2021.pdf",
            "Guide mise en page2021.pdf"
        ]
        
        pdf_url = None
        pdf_path_used = None
        
        for pdf_path in pdf_paths_to_test:
            try:
                pdf_url = get_image_url(pdf_path)
                pdf_path_used = pdf_path
                break
            except:
                continue
        
        if pdf_url:
            st.write(f"✅ PDF trouvé avec le chemin: {pdf_path_used}")
            st.markdown(f"""
            <div style="text-align: center; margin-bottom: 30px;">
            ...
            st.error("PDF non trouvé avec aucun des chemins testés")
            st.markdown("""
            <div style="text-align: center; padding: 40px; background: #f9f9f9; border-radius: 10px; margin: 20px 0;">
    ...
    for i, img_file in enumerate(image_files):
        # Testons plusieurs chemins possibles
        img_paths_to_test = [
            f"SportsWear Design/{img_file}",
            f"mes_documents/SportsWear Design/{img_file}",
            f"mes_documents/{img_file}",
            img_file
        ]
        ...
        for img_path in img_paths_to_test:
            try:
                img_url = get_image_url(img_path)
                img_path_used = img_path
                break
            except:
                continue
        
        if img_url:
            st.write(f"✅ Image {i+1} trouvée avec le chemin: {img_path_used}")
            
            # Afficher l'image en pleine largeur avec une URL CORRECTE
            st.markdown(f"""
            ...
            st.error(f"Image {img_file} non trouvée")
            st.markdown(f"""
            <div style="margin-bottom: 40px;">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
                    <div style="width: 30px; height: 30px; border-radius: 50%; background: #FBBDFA; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;">{i+1}</div>
