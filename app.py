...
if st.session_state.page == "accueil":
    ...
    with col2:
        ...
        if st.button("🔗 Ouvrir dans Google Viewer", use_container_width=True):
            pass
        st.markdown('</a>', unsafe_allow_html=True)) , toi tu va etre le codeur , & moi le designer , & voila l'imager , je veux ou lieu d'afffichier le icone etoil & le mot sher , veux faire , une menu  de degradation comme sa (background-image: radial-gradient(circle at 0% 0%, #fbBDFA 0, transparent 55%), radial-gradient(circle at 100% 0%, #6cd4ff 0, transparent 50%), radial-gradient(circle at 20% 90%, #34d399 0, transparent 55%);) & ne contien , rien , juster , un cercle a deroit , pour maitre dans le le imager de profile , ajouter le menu dans cette code (import streamlit as st
import urllib.parse
